...
public class DevicesController : ControllerBase
{
    ...
    [Authorize(Roles = "Admin,Shopkeeper")]
    public async Task<IActionResult> ReactivateDevice(int id)
    {
        var success = await _deviceService.ReactivateDeviceAsync(id);
        if (!success) return NotFound(new { message = "Device not found or already active." });
